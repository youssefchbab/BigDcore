...
architecture arch of P_Datapath is
    ...
    Signal Reg_file_R2_PLR4: std_logic_vector(31 downto 0);
...
Begin
    ...
    Imm_Extended_PLR: Reg port map(
        clk,reset,
        Imm_Extended,
        Imm_Extended_PLR3
    );
    Reg3_Address_PLR: Reg_5bit port map(
        clk, reset,
        instruction_PLR2(11 downto 7),
        Reg_A3_PLR3
    );
    Reg_file_Register1_PLR3: Reg port map(
        clk, reset,
        Reg_file_R1,
        Reg_file_R1_PLR3
    );
    Reg_file_Register2_PLR3: Reg port map(
        clk, reset,
        Reg_file_R2,
        Reg_file_R2_PLR3
    );
    Pc_PLR_Seocnd: Reg port map(
        clk, reset,
        Pc_Signal_PLR2,
        Pc_Signal_PLR3
    );
    Pc_Plus_Four_PLR_Second: Reg port map(
        clk, reset,
        Pc_Plus_4_PLR2,
        Pc_Plus_4_PLR3
    );
    ...
    Alu_result_PLR: Reg port map(
        clk, reset,
        Alu_result,
        Alu_result_PLR4
    );
    Reg3_Address_PLR_second: Reg_5bit port map(
        clk, reset,
        Reg_A3_PLR3,
        Reg_A3_PLR4
    );
    Reg2_PLR: Reg port map(
        clk, reset,
        Reg_file_R2,
        Reg_file_R2_PLR4
    );
    Pc_Plus_Four_PLR_Third: Reg port map(
        clk, reset,
        Pc_Plus_4_PLR3,
        Pc_Plus_4_PLR4
    );
    ...
    Alu_result_PLR_second: Reg port map(
        clk, reset,
        Alu_result_PLR4,
        Alu_result_PLR5
    );
    Reg3_Address_PLR_Third: Reg_5bit port map(
        clk, reset,
        Reg_A3_PLR4,
        Reg_A3_PLR5
    );
    Read_Data_PLR: Reg port map(
        clk, reset,
        ReadData,
        ReadData_PLR5
    );
    Pc_Plus_Four_PLR_Fourth: Reg port map(
        clk, reset,
        Pc_Plus_4_PLR4,
        Pc_Plus_4_PLR5
    );
