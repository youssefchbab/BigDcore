...
architecture arch of P_Processor_Hazard is
  ...
  Signal Alu_LSB, Zero, DMem_EN, UART_E, MemWE_T: std_logic ;
...
begin
  ...
  MemMap: Memory_Mapping port map(
    address_bus,
    DMem_EN,
    UART_E
  );
  ByteEn<= Alu_LSB_Mem;
  DataAdr<= address_bus;
  MemWrite<= MemWE_T and DMem_EN;
  UART_EN<= UART_E;
  TX_out <= address_bus(7 downto 0);
end arch ; -- arch